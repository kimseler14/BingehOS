...
export function InventoryPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [partData, transactionData] = await Promise.all([
        apiFetch<Part[]>(queryPath("/v1/parts", { skip, take: 12 })),
        apiFetch<InventoryTransaction[]>(queryPath("/v1/inventory/transactions", { skip: 0, take: 30, partId: partIdFilter || undefined })),
      ]);
      setParts(partData);
      setTransactions(transactionData);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Envanter verileri yüklenemedi.");
    } finally {
      setLoading(false);
    }
  }, [partIdFilter, skip]);
