  ...
  return (
    <>
      ...
      <div className="card overflow-hidden">
        {loading ? <div className="flex justify-center py-16 text-teal"><Spinner /></div> : plans.length === 0 ? <EmptyState label="Henüz kat planı bulunmuyor." /> : <div className="divide-y divide-slate-100">{plans.map((plan) => <Link className="flex items-center justify-between px-5 py-5 transition hover:bg-slate-50" href={`/digital-twin?id=${plan.id}`} key={plan.id}><div><p className="font-bold text-ink">{plan.name}</p><p className="mt-1 text-xs text-slate-500">{plan.width} × {plan.height} px · {plan.imageUrl ? "Görsel tanımlı" : "Şematik görünüm"}</p></div><span className="text-sm font-bold text-teal">Planı aç →</span></Link>)}</div>}
      </div>
